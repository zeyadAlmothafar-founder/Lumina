...
### Subtitle
> 5 AI study tools in one app — roadmap, flashcards, Socratic debate, adaptive exam & AI whiteboard, all powered by Gemma 4 31B

### Project Description

> **Lumina** is an open-source AI study suite built entirely on Gemma 4, submitted to the *Future of Education* track of the Gemma 4 Good Hackathon.
>
> The problem it addresses is simple: most students have access to information but not to *structured, interactive, personalised* learning. Tutors are expensive. Study plans are generic. Practice is passive. Lumina changes that by giving every learner five deeply integrated tools in one app.
>
> **Learning GPS** turns any learning goal into a week-by-week roadmap with daily focus areas, free resources, and milestone checkpoints, all generated by Gemma 4. **Flash & Quiz** creates spaced-repetition flashcards from a topic description or from photos of handwritten notes — Gemma 4's vision reads the images and generates cards from them. **AgentDebate** deploys three specialised Gemma 4 agents — a Critical Challenger, a Consensus Builder, and a Wikipedia-backed Fact-Checker — that debate any topic live via server-sent event streaming. The Fact-Checker fetches real Wikipedia articles at session start and cites every claim; the student can interrupt agents mid-round using voice (processed locally by the browser's Web Speech API) or text. The debate ends with a Gemma 4-synthesised essay outline downloadable as a formatted `.docx` file. **The Examiner** conducts an adaptive oral exam where difficulty adjusts after every answer; students can respond by voice, typed text, or a photo of their handwritten answer that Gemma 4 reads via its vision capability. The session ends with a full graded report. **AI Whiteboard** pairs a full Excalidraw drawing canvas with a Gemma 4 chat panel — students draw diagrams or write equations, snapshot the board, and ask Gemma to explain or correct what it sees.
>
> Technically, the app uses Gemma 4 31B IT throughout — with JSON mode for structured outputs, vision for image understanding, multi-turn conversation for adaptive flows, and streaming for live debate generation. Speech recognition runs entirely in the browser via the Web Speech API (no audio leaves the device). The Wikipedia REST API provides grounded knowledge for the Fact-Checker agent at no cost. The frontend is React 18 + Vite + Tailwind CSS with full Arabic RTL support, and the backend is Node.js + Express. The only external credential required to run the app is a free Google AI Studio API key.
